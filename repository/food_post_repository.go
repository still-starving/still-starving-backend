...
func (r *FoodPostRepository) FindAll(status string, lat, lng, radius float64, limit, offset int) ([]models.FoodPost, error) {
	...
	if lat != 0 && lng != 0 && radius > 0 {
		...
		whereClauses = append(whereClauses, fmt.Sprintf("ST_DWithin(fp.location_geo::geography, ST_SetSRID(ST_MakePoint($%d, $%d), 4326)::geography, $%d)", argCount, argCount+1, argCount+2))
