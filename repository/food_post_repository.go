...
func (r *FoodPostRepository) Create(post *models.FoodPost) error {
	...
	query := `
		INSERT INTO food_posts (id, user_id, title, description, quantity, location, expiry_date, price, currency, spice_level, ingredients, cooked_at)
...
func (r *FoodPostRepository) Update(post *models.FoodPost) error {
	query := `
		...
		SET title = $1, description = $2, quantity = $3, location = $4,
		    ...
		    spice_level = $9, ingredients = $10, cooked_at = $11, updated_at = CURRENT_TIMESTAMP
		WHERE id = $12
	...
	return r.db.QueryRow(
		...
		post.SpiceLevel, post.Ingredients, post.CookedAt, post.ID,
	).Scan(&post.UpdatedAt)
}
